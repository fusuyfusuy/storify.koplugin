...
local USER_AGENT = "KOReader-Storify"

local function safeUrlEscape(s)
    if socket_url and type(socket_url.escape) == "function" then
        return socket_url.escape(s)
    end
...
local function safeUrlParse(u)
    if socket_url and type(socket_url.parse) == "function" then
        local p = socket_url.parse(u)
        if p and p.host then
            return p
        end
    end
    if not u then return nil end
    local scheme, rest = u:match("^([a-zA-Z0-9+.-]+)://(.*)$")
...
local function safeUrlAbsolute(base, rel)
    if not rel or rel == "" then return base end
    if rel:find("^https?://") then return rel end
    if socket_url and type(socket_url.absolute) == "function" then
        local ok, abs = pcall(socket_url.absolute, base, rel)
...
local function getConfig()
    local ok_cfg, cfg = pcall(require, "storify_configuration")
...
local function getAuthHeaders()
    local cfg = getConfig()
    local auth = cfg.auth and cfg.auth.github
    if not auth then
        return nil
    end
...
--
-- Timeouts and error handling are delegated to Net.requestToTable, which wraps
-- every attempt in pcall and resets socketutil timeouts on exit.
local function requestWithGuardedRedirects(target, headers)
    local current = target
    for _ = 0, MAX_REDIRECTS do
        local parsed = safeUrlParse(current)
        if not parsed then
        ...
        if (parsed.scheme or ""):lower() ~= "https" then
            logger.warn("Storify: refusing non-https URL", current)
            return 0, "refusing non-https URL"
        end
        if not ALLOWED_HOSTS[host] then
            logger.warn("Storify: refusing unexpected host", host)
        ...
        local response_body = {}
        local code, resp_headers, status = Net.requestToTable({
            url = current,
        ...
        if num_code == 301 or num_code == 302 or num_code == 303 or num_code == 307 or num_code == 308 then
            local location = resp_headers and (resp_headers.location or resp_headers.Location)
            if not location or location == "" then
                return num_code or code, table.concat(response_body)
            end
            current = safeUrlAbsolute(current, location)
        else
            local body = table.concat(response_body)
            return num_code or code, body
        end
    end
    logger.warn("Storify: too many redirects for", target)
...
local function request(path, query)
    ...
    logger.dbg("Storify HTTP", target)
    local headers = {
...
local function buildQuery(opts)
    ...
    if opts.q and opts.q ~= "" then
        table.insert(query_parts, "q=" .. safeUrlEscape(opts.q))
    end
