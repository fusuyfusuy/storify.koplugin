--[[--
Self-contained localization for the Storify plugin.
...
local function thisDir()
    ...
    -- source looks like "@C:\path\to\storify.koplugin\storify_gettext.lua"
    return source:match("^@(.*)[/\\][^/\\]+$")
end
...
local function loadLangTable(lang)
    ...
    local function tryCode(code)
        ...
        logger.warn("storify_gettext: failed to load", path, tbl)
        return nil
    end
...
local StorifyGetText = setmetatable({}, {
